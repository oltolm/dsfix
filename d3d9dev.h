...
private:
  ...
  Microsoft::WRL::ComPtr<IDirect3D9> m_pD3Dint;
  std::unique_ptr<Ui> m_ui;
  std::unique_ptr<RSManager> m_rsManager;
public:
  hkIDirect3DDevice9(IDirect3DDevice9 * pIDirect3DDevice9, IDirect3D9 * pIDirect3D9);
  virtual ~hkIDirect3DDevice9() = default;
  float getOcclusionScale() const { return m_rsManager->getOcclusionScale(); }
  ...
  (UINT StreamNumber, IDirect3DVertexBuffer9 * *ppStreamData, UINT * pOffsetInBytes,
   UINT * pStride);
