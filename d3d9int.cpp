...
ULONG APIENTRY hkIDirect3D9::AddRef() { return m_pD3Dint->AddRef(); }

HRESULT APIENTRY hkIDirect3D9::CheckDepthStencilMatch(UINT Adapter, D3DDEVTYPE DeviceType,
                                                      D3DFORMAT AdapterFormat,
                                                      D3DFORMAT RenderTargetFormat,
                                                      D3DFORMAT DepthStencilFormat) {
  return m_pD3Dint->CheckDepthStencilMatch(Adapter, DeviceType, AdapterFormat, RenderTargetFormat,
                                           DepthStencilFormat);
}
...
HRESULT APIENTRY hkIDirect3D9::CheckDeviceFormat(UINT Adapter, D3DDEVTYPE DeviceType,
  ...
  return m_pD3Dint->CheckDeviceFormat(Adapter, DeviceType, AdapterFormat, Usage, RType,
                                      CheckFormat);
}
...
HRESULT APIENTRY hkIDirect3D9::CheckDeviceMultiSampleType(UINT Adapter, D3DDEVTYPE DeviceType,
  ...
  return m_pD3Dint->CheckDeviceMultiSampleType(Adapter, DeviceType, SurfaceFormat, Windowed,
...
HRESULT APIENTRY hkIDirect3D9::CreateDevice(UINT Adapter, D3DDEVTYPE DeviceType, HWND hFocusWindow,
  ...
  if (pPresentationParameters) {
    adjusted = RSManager::get().adjustPresentationParameters(pPresentationParameters);
    pPresentationParameters = &adjusted;
  }
  HRESULT hRet = m_pD3Dint->CreateDevice(Adapter, DeviceType, hFocusWindow, BehaviorFlags,
                                         pPresentationParameters, ppReturnedDeviceInterface);
  if (SUCCEEDED(hRet))
    *ppReturnedDeviceInterface = new hkIDirect3DDevice9(*ppReturnedDeviceInterface, this);
  return hRet;
}
...
UINT APIENTRY hkIDirect3D9::GetAdapterCount() { return m_pD3Dint->GetAdapterCount(); }
...
ULONG APIENTRY hkIDirect3D9::Release() { return m_pD3Dint->Release(); }
